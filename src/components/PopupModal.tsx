      ...
      <div className="relative w-full max-w-lg max-h-[92vh] flex flex-col bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isAdminUnlocked ? (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 active:scale-95 text-white font-bold text-xs rounded-xl transition-all shadow-xs"
                title="관리자 수정 모드 실행"
              >
              ...
              <div className="flex items-center gap-1.5">
                <span className="px-2.5 py-1 bg-emerald-500 text-white text-[10px] font-black rounded-lg shadow-2xs flex items-center gap-1">
                  <Check className="w-3 h-3" /> 관리자 인증됨
                </span>
                <button
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {visiblePopups.map((popup) => {
            ...
            return (
              ...
              >
                {/* Admin Quick Control Bar on Card */}
        ...
        <div className="px-6 py-3 bg-slate-100 border-t border-slate-200 flex items-center justify-between">
          <button
            ...
            className="text-xs text-slate-500 hover:text-orange-600 font-bold flex items-center gap-1 transition-colors cursor-pointer"
          >
            <Shield className="w-3.5 h-3.5" />
            <span>{isAdminUnlocked ? '전체 관리자 센터' : '관리자 로그인'}</span>
          </button>
